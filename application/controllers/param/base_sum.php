...
class Base_Sum extends CI_Controller {
    ...
    {
        $data['wrapper'] = 'Tabela de Regras / Base de Calculo';
        $obj = new stdClass();
        ...
        $obj->table_manager->fields = $this->DB_Regras_Base->get_fields();
        $obj->table_manager->result = $this->DB_Regras_Base->get_all($sta,$end);     
    ...
    {
        $script = $this->load->file(APPPATH . 'hooks/js_base_sum.php');
        return $script;
    }  
    ...
    public function set_page_table()
    {
        ...
        $end = 5 * $this->input->post('regra_page');
        $sta = $end - 5;  
    ...
    public function save($regra_id=-1)
    {        
        if($this->DB_Regras_Base->save($this->input->post(), $regra_id))
        {
    ...
    {               
        $regra_ids = $this->input->post('regra_ids');        
        if($this->DB_Regras_Base->delete($regra_ids))
        {
            $html ='<div class="alert ng-isolate-scope alert-success alert-dismissable" role="alert" ng-class="[\'alert-\' + (type || \'warning\'), closeable ? \'alert-dismissable\' : null]" close="closeAlert($index)" type="success" ng-repeat="alert in alerts">'.PHP_EOL;
            $html .='<button class="close" onclick="window.location.reload();" type="button"><span aria-hidden="true">x</span><span class="sr-only">Close</span></button>'.PHP_EOL;
            $html .='<div ng-transclude=""><span class="ng-binding ng-scope">Sucesso! O registro foi processado com êxito.</span></div>';
    ...
    {   
        $regra_data = array('regras_deletado' => $this->input->post('regras_deletado'));             
        if($this->DB_Regras_Base->save($regra_data, $this->input->post('regra_id')))
        {
            $html ='<div class="alert ng-isolate-scope alert-success alert-dismissable" role="alert" ng-class="[\'alert-\' + (type || \'warning\'), closeable ? \'alert-dismissable\' : null]" close="closeAlert($index)" type="success" ng-repeat="alert in alerts">'.PHP_EOL;
            $html .='<button class="close" onclick="window.location.reload();" type="button"><span aria-hidden="true">x</span><span class="sr-only">Close</span></button>'.PHP_EOL;
            $html .='<div ng-transclude=""><span class="ng-binding ng-scope">Sucesso! O registro foi processado com êxito.</span></div>';
...
/* End of file basesum.php */
/* Location: ./application/controllers/param/basesum.php */