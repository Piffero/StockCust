...
class RD_Raw_Material extends CI_Controller
{
	...
	function index()
	{
	    ...
	    $obj = new stdClass();   
	    $obj->showFields =  5;
	    $obj->fields = $this->DB_Raw_Material->get_fields();
        $obj->result = $this->DB_Raw_Material->get_all(0,1000);      
	    ...
	    $pdf->SetTitle('Matérias-Primas Registradas');
	    ...
	    $PDF_HEADER_TITLE = 'Registro de Matérias-Primas';
	    ...
	    $pdf->writeHTML('<br><h5>Lista de Matérias-Primas<hr></h5>');
	    $pdf->SetFontSize(7);
	    $pdf->writeHTML($table);
	...
	{	
        ...
        $html .= '                                <thead>'.PHP_EOL;
        $html .= '                                    <tr>'.PHP_EOL;        
        $html .= '                                        <th><strong>Matéria-Prima</strong></th>'.PHP_EOL;
        $html .= '                                        <th><strong>Unidade</strong></th>'.PHP_EOL;
        $html .= '                                        <th><strong>Preço Custo</strong></th>'.PHP_EOL;
        $html .= '                                        <th><strong>FC</strong></th>'.PHP_EOL;
        $html .= '                                    </tr>'.PHP_EOL;
        $html .= '                                </thead>'.PHP_EOL;
        $html .= '                                <tbody>'.PHP_EOL;       
