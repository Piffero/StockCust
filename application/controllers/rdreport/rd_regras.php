...
class RD_Regras extends CI_Controller
{
	...
	function index()
	{
	    ...
	    $obj = new stdClass();   
	    $obj->showFields =  3;
	    $obj->fields = $this->DB_Regras_Base->get_fields();
        $obj->result = $this->DB_Regras_Base->get_all(0,1000);      
	    ...
	    $pdf->SetTitle('Regras Registradas');
	    ...
	    $PDF_HEADER_TITLE = 'Registro de Regras';
	    ...
	    $pdf->writeHTML('<br><h5>Lista de Regras do Sistema<hr></h5>');
	    $pdf->SetFontSize(7);
	    $pdf->writeHTML($table);
	...
	{	
        ...
        $html .= '                                        <th><strong>Exibição</strong></th>'.PHP_EOL;
        $html .= '                                        <th><strong>Base Calculo</strong></th>'.PHP_EOL;
        $html .= '                                        <th><strong>Situação</strong></th>'.PHP_EOL;
        $html .= '                                    </tr>'.PHP_EOL;
        $html .= '                                </thead>'.PHP_EOL;
        $html .= '                                <tbody>'.PHP_EOL;       
    ...
    private function create_object_data_row($field, $fields, $show)
    {
     ...
     for($i=0; $i < $show; $i++){
        ...
        if(($field->$fields[4] == 0)){ $html .= 'Ativo'.PHP_EOL; } else { $html .= 'Desativado'.PHP_EOL; }
